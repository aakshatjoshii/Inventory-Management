...
if(isset($_POST["issue_book_button"]))
{
    ...
    if($error == '')
    {
        ...
        WHERE product_id_number = '".$formdata['book_id']."'
        ";
        ...
        if($statement->rowCount() > 0)
        {
            foreach($statement->fetchAll() as $book_row)
            {
                ...
                if($book_row['product_status'] == 'Enable' && $book_row['product_no_of_item'] > 0)
                {
                    //Check User is exist
                        ...
                        foreach($statement->fetchAll() as $user_row)
                        {
                            if($user_row['user_status'] == 'Enable')
                            {
                                ...
                                if($total_book_issue < $book_issue_limit)
                                {
                                    ...
                                    (book_id, user_id, location_name , floor_name , room_no , issue_date_time, expected_return_date, return_date_time, book_fines, book_issue_status) 
                                    VALUES (:book_id, :user_id, :location_name , :floor_name , :room_no , :issue_date_time, :expected_return_date, :return_date_time, :book_fines, :book_issue_status)
                                    ";
                                    ...
                                    $query = "
                                    UPDATE lms_book 
                                    SET product_no_of_item = product_no_of_item - 1, 
                                    product_updated_on = '".$today_date."' 
                                    WHERE product_id_number = '".$formdata['book_id']."' 
                                    ";
                                ...
                                {
                                    $error .= 'User has already reached Book Issue Limit, First return pending items';
                                }
                            }
                            else
                            {
                ...
                {
                    $error .= '<li>Product not Available</li>';
                }
            }
        }
        else
        {
            $error .= '<li>Product not Found</li>';
        }
    }
}
...
if(isset($_POST["book_return_button"]))
{
    if(isset($_POST["book_return_confirmation"]))
    {
        ...
        UPDATE lms_book 
        SET product_no_of_item = product_no_of_item + 1 
        WHERE product_id_number = '".$_POST["product_id_number"]."'
        ";
    ...
    {
        $error = 'Please first confirm return product received by click on checkbox';
    }
}   
...
<div class="container-fluid py-4" style="min-height: 700px;">
	<h1>Issue Product Management</h1>
    <?php 
                ...
                <div class="card-body">
                    ...
                    book_id.onkeyup = function()
                    {
                        if(this.value.length > 2)
                        {
                            ...
                            fetch('action.php', {
                                ...
                                {
                                    html += '<a href="#" class="list-group-item list-group-item-action">No Product Found</a>';
                                }
                    ...
                    user_id.onkeyup = function(){
                        ...
                        {   
                            ...
                            form_data.append('action', 'search_location_name');
                            ...
                            fetch('action.php', {
                                ...
                                if(responseData.length > 0)
                                {
                                    for(var count = 0; count < responseData.length; count++)
                                    {
                                        html += '<a href="#" class="list-group-item list-group-item-action"><span onclick="get_text1(this)">'+responseData[count].location_name+'</span> - <span class="text-muted">'+responseData[count].floor_name+'</span> - <span class = "text-muted">'+response[count].room_no'</a>';
                                    }
                                }
                                else
                                {
                                ...
                                document.getElementById('location_name_result').innerHTML = html;
                            });
                        }
                        else
                        {
                            document.getElementById('location_name_result').innerHTML = '';
                        }
                    }
        ...
        else if($_GET["action"] == 'view')
        {
            ...
            if($issue_book_id > 0)
            {
                ...
                foreach($result as $row)
                {
                    $query = "
                    SELECT * FROM lms_book 
                    WHERE product_id_number = '".$row["book_id"]."'
                    ";
                    ...
                    WHERE location_name = '".$row["location_name"]."' AND floor_name = '".$row["floor_name"]."' AND room_no = '".$row["room_no"]."'
                    ";
                    ...
                    <ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
                        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="issue_book.php">Issue Product Management</a></li>
                        <li class="breadcrumb-item active">View Issue Product Details</li>
                    </ol>
                    ';
                    ...
                    foreach($book_result as $book_data)
                    {
                        ...
                        <table class="table table-bordered">
                            <tr>
                                <th width="30%">Product ID Number</th>
                                <td width="70%">'.$book_data["product_id_number"].'</td>
                            </tr>
                            <tr>
                                <th width="30%">Product Title</th>
                                <td width="70%">'.$book_data["product_name"].'</td>
                            </tr>
                            <tr>
                    ...
                    foreach($location_result as $location_data)
                    {
                        echo '
                        ...
                        <table class="table table-bordered">
                            <tr>
                                ...
                                <td width="70%">'.$location_data["location_name"].'</td>
                            </tr>
                            <tr>
                                <th width="30%">Floor Name</th>
                                <td width="70%">'.$location_data["floor_name"].'</td>
                            </tr>
                            <tr>
                                <th width="30%">Room No</th>
                                <td width="70%">'.$location_data["room_no"].'</td>
                            </tr>
                            </tr>
                        </table>
                        <br />
                    ...
                    if($status == "Issue")
                    {
                        ...
                        $form_item = '
                        <label><input type="checkbox" name="book_return_confirmation" value="Yes" /> I aknowledge that I have received Issued Product</label>
                        <br />
                        <div class="mt-4 mb-4">
                    ...
                    if($status == 'Not Return')
                    {
                        ...
                        $form_item = '
                        <label><input type="checkbox" name="book_return_confirmation" value="Yes" /> I aknowledge that I have received Issued Product</label><br />
                        <div class="mt-4 mb-4">
                            <input type="submit" name="book_return_button" value="Book Return" class="btn btn-primary" />
                        </div>
                        ';
                    ...
                    echo '
                    <h2>Issue Product Details</h2>
                    <table class="table table-bordered">
                        <tr>
                            <th width="30%">Product Issue Date</th>
                            <td width="70%">'.$row["issue_date_time"].'</td>
                        </tr>
                        <tr>
                            <th width="30%">Product Return Date</th>
                            <td width="70%">'.$row["return_date_time"].'</td>
                        </tr>
                        <tr>
                            <th width="30%">Product Issue Status</th>
	...
	<ol class="breadcrumb mt-4 mb-4 bg-light p-2 border">
		<li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
        <li class="breadcrumb-item active">Issue Product Management</li>
    </ol>
    ...
    if(isset($_GET['msg']))
    {
        if($_GET['msg'] == 'add')
        {
            echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Product Issued Successfully<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
        }

        if($_GET["msg"] == 'return')
        {
            echo '
            <div class="alert alert-success alert-dismissible fade show" role="alert">Issued Product Successfully Returned <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
            ';
        }
    }
    ?>
    	...
    	<div class="card-header">
    		<div class="row">
    			<div class="col col-md-6">
    				<i class="fas fa-table me-1"></i> Issue Product Management
                </div>
